# Roadmap: Sakhr Story Animation

## Overview

Complete the 4-chapter animated storytelling system for Sakhr MSX history. Starting with Chapter 1's remaining scenes, then building out chapters 2-4, culminating in performance optimization for smooth Raspberry Pi playback.

## Domain Expertise

None - Vanilla JavaScript animation with established patterns

## Phases

**Phase Numbering:**
- Integer phases (1, 2, 3, 4): Planned milestone work
- Decimal phases (2.1, 2.2): Urgent insertions (marked with INSERTED)

- [ ] **Phase 1: Complete Chapter 1** - Finish remaining scenes (sc6+) to complete first chapter
- [ ] **Phase 2: Chapter 2** - Create all scenes for second chapter
- [ ] **Phase 3: Chapter 3** - Create all scenes for third chapter
- [ ] **Phase 4: Chapter 4 & Final Polish** - Complete final chapter, optimize for Raspberry Pi

## Phase Details

### Phase 1: Complete Chapter 1
**Goal**: Finish all remaining scenes in Chapter 1 (starting with sc6) and verify chapter plays smoothly
**Depends on**: Nothing (engine complete, scenes 1-5 done)
**Research**: Unlikely (following existing scene patterns from sc1-sc5)
**Plans**: 1-2 plans

Plans:
- [x] 01-01: Create sc6 scene from prompt (scene.json, HTML player, add to scenes.json)

### Phase 2: Chapter 2
**Goal**: Create complete Chapter 2 with all scenes
**Depends on**: Phase 1
**Research**: Unlikely (same patterns as Chapter 1)
**Plans**: 2-3 plans

Plans:
- [ ] 02-01: Plan chapter 2 structure and create initial scenes
- [ ] 02-02: Complete remaining chapter 2 scenes

### Phase 3: Chapter 3
**Goal**: Create complete Chapter 3 with all scenes
**Depends on**: Phase 2
**Research**: Unlikely (established workflow)
**Plans**: 2-3 plans

Plans:
- [ ] 03-01: Plan chapter 3 structure and create initial scenes
- [ ] 03-02: Complete remaining chapter 3 scenes

### Phase 4: Chapter 4 & Final Polish
**Goal**: Complete final chapter and optimize entire 4-chapter experience for Raspberry Pi
**Depends on**: Phase 3
**Research**: Unlikely (performance testing on target hardware)
**Plans**: 2-3 plans

Plans:
- [ ] 04-01: Complete Chapter 4 scenes
- [ ] 04-02: Performance optimization and Raspberry Pi testing

## Progress

| Phase | Plans Complete | Status | Completed |
|-------|----------------|--------|-----------|
| 1. Complete Chapter 1 | 1/1-2 | In progress | - |
| 2. Chapter 2 | 0/2-3 | Not started | - |
| 3. Chapter 3 | 0/2-3 | Not started | - |
| 4. Chapter 4 & Polish | 0/2-3 | Not started | - |
